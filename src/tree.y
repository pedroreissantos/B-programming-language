...
%type<n> def vc inits names ival cte name cnames enames stmt stmts retval lvalue rvalue args decl
...
%token LIST STMT BLOCK ALLOC INDEX CALL ARIF ARELSE BNOT FUNC GLOBAL BODY ENTER VAR INIT FNAME
%%
...
def	: name vc inits ';'		{ IDnew($2->info, $1->value.s, 0);
	    $$ = binNode(VAR, uniNode(GLOBAL, $1), binNode(INIT, $2, $3)); }
	| name vc ';'			{ IDnew($2->info, $1->value.s, 0);
	    $$ = binNode(VAR, uniNode(GLOBAL, $1), binNode(INIT, $2, nilNode(END))); }
	| name '(' { IDnew(1, $1->value.s, 0); IDpush(); pos = 8; } names ')' { pos = 0; } stmt { gotos(); printf("# %s: enter %d\n", $1->value.s, -pos); IDpop();
	  $$ = binNode(FUNC, uniNode(GLOBAL, $1), binNode(BODY, intNode(ENTER, -pos), $7)); }
	| name '(' { IDnew(1, $1->value.s, 0); IDpush(); pos = 0; } ')' stmt { gotos(); printf("# %s: enter %d\n", $1->value.s, -pos); IDpop();
	  $$ = binNode(FUNC, uniNode(GLOBAL, $1), binNode(BODY, intNode(ENTER, -pos), $5)); }
	;
...
inits	: ival				{ $$ = binNode(LIST, nilNode(END), $1); }
	| inits ',' ival		{ $$ = binNode(LIST, $1, $3); }
	;
...
enames	: name				{ $$ = binNode(EXTRN, nilNode(END), $1); IDnew(0, $1->value.s, 0); $1->info = 0; }
	| name '(' ')'			{ $$ = binNode(EXTRN, nilNode(END), $1); IDnew(1, $1->value.s, 0); $1->info = 1; $$->CHILD(1)->attrib = FNAME; }
	| enames ',' name		{ $$ = binNode(EXTRN, $1, $3); IDnew(0, $3->value.s, 0); $3->info = 0; }
	| enames ',' name '(' ')'	{ $$ = binNode(EXTRN, $1, $3); IDnew(1, $3->value.s, 0); $3->info = 1; $$->CHILD(1)->attrib = FNAME; }
	;
...
rvalue	: lvalue		{ $$ = uniNode(LOAD, $1); }
	| '(' rvalue ')'	{ $$ = $2; }
	| cte
	| rvalue '(' args ')'	{ $$ = binNode(CALL, $3, $1); }
	| rvalue '(' ')'	{ $$ = binNode(CALL, nilNode(END), $1); }
